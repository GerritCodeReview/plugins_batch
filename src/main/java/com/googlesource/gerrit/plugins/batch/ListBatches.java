...
public class ListBatches implements RestReadView<TopLevelResource> {
  @Argument(index = 0, required = false, multiValued = true, metaVar = "QUERY",
      usage = "Query to execute")
  public List<String> query;

  @Option(name = "--include-last-modified",
  ...
  public Object apply(TopLevelResource resource) throws Exception {
    return OutputFormat.JSON.newGson().toJsonTree(getBatches(),
          new TypeToken<List<Object>>() {}.getType());
  }

  public void display(OutputStream displayOutputStream) throws IOException,
      OrmException, QueryParseException {
    try {
      PrintWriter stdout = new PrintWriter(new BufferedWriter(
          new OutputStreamWriter(displayOutputStream, "UTF-8")));
      try {
        OutputFormat.JSON.newGson().toJson(getBatches(),
            new TypeToken<List<Batch>>() {}.getType(), stdout);
        stdout.print('\n');
      } finally {
        stdout.flush();
      }
