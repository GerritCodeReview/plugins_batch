...
public class BatchQueryBuilder extends QueryBuilder<Batch> {
  ...
  private static final QueryBuilder.Definition<Batch, BatchQueryBuilder> mydef =
      new QueryBuilder.Definition<Batch, BatchQueryBuilder>(BatchQueryBuilder.class);

  private static final long DEFAULT_SECONDS = TimeUnit.SECONDS.convert(3, TimeUnit.DAYS);
  ...
  public BatchQueryBuilder(
      ProjectCache projectCache, PluginConfigFactory pluginConfigFactory, BatchStore store) {
    super(mydef);
  ...
  public Date getExpiry() throws NoSuchProjectException {
    Config cfg = pluginConfigFactory.getProjectPluginConfig(projectCache.getAllProjects(), "batch");
    long seconds =
        ConfigUtil.getTimeUnit(cfg, "cleaner", null, "maxAge", DEFAULT_SECONDS, TimeUnit.SECONDS);
    long ms = TimeUnit.MILLISECONDS.convert(seconds, TimeUnit.SECONDS);
    return new Date(new Date().getTime() - ms);
  }

  @Operator
  public Predicate<Batch> expired(final String val) throws NoSuchProjectException {
