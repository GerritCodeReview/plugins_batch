...
result_out "$GROUP submit approval" "SUBM" \
    "$(query_by "$(query --all-approvals "$ch1" | grep SUBM)" "type")"
...
result "$GROUP" "$delete"
error=$(batchssh submit --force "$id")
result_out "$GROUP submit after delete" "No such batch: $id" "$error"
...
ch1=$(create_change "$REF_BRANCH" "$FILE_A") || exit
bjson=$(batchssh merge-change --close "$ch1",1)
list=$(batchssh ls-batches --include-last-modified)
...
setupGroup "dependent changes merge" "Dependent changes, requires merge" # --

ch1=$(create_change "$REF_BRANCH" "$FILE_A") || exit
ch2=$(create_change --dependent "$REF_BRANCH" "$FILE_A") || exit
qchange=$(query "$ch2" --current-patch-set)
qsha1=$(query_by "$qchange" "revision") || exit

chb=$(create_change "$REF_BRANCH" "$FILE_B") || exit
bjson=$(batchssh merge-change --close "$chb",1)
b_id=$(b_id)
b_qchange=$(query "$chb" --current-patch-set)
b_sha1=$(query_by "$b_qchange" "revision") || exit
#         +- chb
#  HEAD <-+
#         +- ch1 <- ch2

junk=$(batchssh submit --force "$b_id") || exit
#             +- chb (HEAD)
#  old HEAD <-+
#             +- ch1 <- ch2
#
...
#              +- ch1 <- ch2 <-+
#
parents=$(get_ref_parents "$dw_ref")
result_out "$GROUP" "$b_sha1 $qsha1" "$parents"
...
dw_ref=$(d_download "$dest1")
parents=$(get_ref_parents "$dw_ref")
result_out "$GROUP reverse" "$b_sha1 $qsha1" "$parents"
